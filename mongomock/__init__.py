...
def _print_deprecation_warning(old_param_name, new_param_name):
    warnings.warn("""
        '{}' has been deprecated to be in line with pymongo implementation, a
        new parameter '{}' should be used instead. the old parameter will be
        kept for backwardcompatibility purposes."""\
        .format(old_param_name, new_param_name, DeprecationWarning))
...
class Collection(object):
            ...
            for k, v in iteritems(document):
                if k == '$set':
                    ...
                    for field, value in v.iteritems():
                        if value and existing_document in field:
                    ...
                    for field, value in iteritems(v):
                        ...
                        existing_document[field] = \
                            [obj for obj in arr if not obj == value]
                else:
                    if first:
                        ...
                        _id = spec.get('_id',
                                       existing_document.get('_id', None))
    ...
    def find(self, spec=None, fields=None, filter=None):
        if filter is not None:
            ...
            if spec is None:
                spec = filter
        dataset = (self._copy_only_fields(document, fields)\
    ...
    def _iter_documents(self, filter=None):
        return (document for document in itervalues(self._documents)\
                         if self._filter_applies(filter, document))
    ...
    def remove(self, spec_or_id=None, safe=None, search_filter=None):
